...
之后,我们决定用一个几何模型 [fn:4]。参照罚点球的示意图,再加上一点三角学知识,可以得到如下
命中概率公式:
...
#+NAME: ch02-code-25606749149d86af
#+BEGIN_SRC python
%matplotlib inline
import os
...
#+NAME: ch02-code-c224ec9d542c1799
#+BEGIN_SRC python
...
#+NAME: ch02-code-f1df5f006fc75a0a
#+BEGIN_SRC python
...
#+NAME: ch02-code-45a2834be02a1223
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-45a2834be02a1223-43c75b2c75-figure.png]]
...
#+NAME: ch02-code-922d880020a0286f
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-922d880020a0286f-6739b0f61e-figure.png]]
...
#+NAME: ch02-code-4dfa77c3467d38e9
#+BEGIN_SRC python
...
#+NAME: ch02-code-12b27b78e296472a
#+BEGIN_SRC python
...
#+NAME: ch02-code-da1997ce8d05c1c2
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-da1997ce8d05c1c2-85cb0bf3ac-figure.png]]
...
#+NAME: ch02-code-1b0254669d091359
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-1b0254669d091359-f629cadae8-figure.png]]
...
#+NAME: ch02-code-6ef780da8540df2a
#+BEGIN_SRC python
_, ax = plt.subplots(1, 2, figsize=(10, 4))
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-6ef780da8540df2a-024c7d72c2-figure.png]]
...
前提是你要记住:你能改变的是*模型*,而不是观测数据 [fn:8]。同样从上到下:
...
#+NAME: ch02-code-f67edca1a877b268
#+BEGIN_SRC python
n_obs = 500
samples = 2000
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-f67edca1a877b268-ce5cd74b61-figure.png]]
...
#+NAME: ch02-code-070375c7b8fe5e67
#+BEGIN_SRC python
np.random.seed(5201)
#+END_SRC

#+NAME: ch02-code-60d0009f16ec945d
#+BEGIN_SRC python
...
#+NAME: ch02-code-997bc919af98ccfc
#+BEGIN_SRC python
az.ess(chains)
#+END_SRC
#+RESULTS: output
...
#+NAME: ch02-code-961c3ffd060f32a1
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-961c3ffd060f32a1-16c5ef179a-figure.png]]
...
#+NAME: ch02-code-e693124a2d91056b
#+BEGIN_SRC python
az.rhat(chains)
#+END_SRC
#+RESULTS: output
...
#+NAME: ch02-code-ef2be5c414df2fae
#+BEGIN_SRC python
az.mcse(chains)
#+END_SRC
#+RESULTS: output
...
#+NAME: ch02-code-12f4dc6a4179c6fc
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-12f4dc6a4179c6fc-7cbbf5d4b9-figure.png]]


最后,ESS、$\hat R$ 和 MCSE 都可以通过一次调用 =az.summary(.)= 函数得到:

#+NAME: ch02-code-bab87765ecbacaa0
#+BEGIN_SRC python
...
表格第一列是均值(期望)的蒙特卡洛标准误,第二列是标准差的蒙特卡洛标准误 [fn:10]。接下来是
...
#+NAME: ch02-code-becd4c9a8a4677af
#+BEGIN_SRC python
az.plot_trace(chains)
plt.savefig("img/chp02/trace_plots.png")
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-becd4c9a8a4677af-8df60192fb-figure.png]]
...
#+NAME: ch02-code-59a4d0dc48113519
#+BEGIN_SRC python
az.plot_autocorr(chains, combined=True, figsize=(12, 4))
plt.savefig('img/chp02/autocorrelation_plot.png')
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-59a4d0dc48113519-12f7d586f6-figure.png]]
...
#+NAME: ch02-code-4b42393695d2de7a
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-4b42393695d2de7a-e1b397cb27-figure.png]]


"柱状"表示法之外的一种替代方案,是用竖线来表示,简称"vlines"。

#+NAME: ch02-code-274b556c5f4199be
#+BEGIN_SRC python
az.plot_rank(chains, kind="vlines", figsize=(12, 4))
plt.savefig('img/chp02/rank_plot_vlines.png')
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-274b556c5f4199be-695ec60034-figure.png]]
...
概念 [fn:11]。发散(更准确地说是"发散转移",divergent transitions),是诊断样本的一种强大而
...
#+NAME: ch02-code-44296a065b3763ec
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-dc8e7ddf07b69054-2ceac14429-figure.png]]

#+NAME: ch02-code-9d54649b307dc44b
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-9d54649b307dc44b-cb5de74e2c-figure.png]]
...
改进模型结构、使用信息量更大的先验,乃至干脆更换模型,往往会更有效 [fn:13]。值得一提的是,在
...
不论是连续还是离散模型,我们都统一用"密度"这个词来称呼 [fn:14]。
...
有使用 [fn:15]。当似然是高斯分布时,这个量会正比于均方误差。
...
#+NAME: ch02-code-f972d11031f579dd
#+BEGIN_SRC python
np.random.seed(90210)
#+END_SRC

#+NAME: ch02-code-561fdbe8b5d67c73
#+BEGIN_SRC python
...
要计算 LOO,我们只需要后验样本 [fn:16]。之后就可以调用 =az.loo(.)=,它可以为单个模型计算 LOO。
实践中更常见的是同时对两个或更多模型计算 LOO,因此更常用的是 =az.compare(.)= 函数。下表就是
用 =az.compare(idatas_cmp)= 生成的。

#+NAME: ch02-code-b0659db14e138394
#+BEGIN_SRC python
...
#+NAME: ch02-code-0b2213623b14889b
#+BEGIN_SRC python
az.plot_compare(cmp, figsize=(9, 3))
plt.savefig("img/chp02/compare_dummy.png")
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-0b2213623b14889b-70332deccb-figure.png]]
...
#+NAME: ch02-code-3b1458abe2554be8
#+BEGIN_SRC python
az.plot_elpd(idatas_cmp, figsize=(10, 5), plot_kwargs={"marker":"."}, threshold=2);
plt.savefig("img/chp02/elpd_dummy.png")
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-3b1458abe2554be8-0ade60d9f8-figure.png]]
...
- 使用矩匹配(moment matching)方法 [fn:17]。通过一些额外计算,可以把 MCMC 后验抽样转换成
  更可靠的重要性采样估计。
...
#+NAME: ch02-code-60279f4754c0686a
#+BEGIN_SRC python
_, axes = plt.subplots(1, 3, figsize=(12, 4), sharey=True)
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-60279f4754c0686a-c864295fa5-figure.png]]

#+NAME: ch02-code-aa7ec06d6e9af8a9
#+BEGIN_SRC python
...
#+END_EXAMPLE
[[file:generated/chapter-2-93b35cf98b-ch02-code-aa7ec06d6e9af8a9-d2c7f43f39-figure.png]]
...
- 如果 $p_loo \ll p$,模型很可能被误设了。通常在后验预测检验中也会看到问题——后验预测
  样本与观测值匹配得很差。
- 如果 $p_loo < p$,且 $p$ 相对观测数量而言比较大(比如 $p > \frac{N}{5}$,$N$ 是观测总
  数),这通常说明模型过于灵活,或者先验信息量不足,导致很难预测被留出的那个观测值。
- 如果 $p_loo > p$,模型同样可能存在严重的误设。如果参数个数 $p \ll N$,后验预测检验也
...
Transform)[fn:19]——这是一种一维变换,如果用任意连续随机变量自身的 CDF 对其做变换,就能得到
...
听起来耳熟,是因为这其实正是我们在"理解你的预测"一节中,借助 ~az.plot_bpv(idata, kind="u_value")~ 讨论过的同一个想法。
...
#+NAME: ch02-code-8399e29163c4df10
#+BEGIN_SRC python
_, axes = plt.subplots(1, 3, figsize=(12, 4), sharey=True)
...
#+END_SRC
[[file:generated/chapter-2-93b35cf98b-ch02-code-8399e29163c4df10-4d1ac13ea4-figure.png]]
...
权重法 [fn:20],是一种从 LOO 出发、启发式地计算每个模型(在给定这一组模型的前提下)相对概率
的方法 [fn:21]。分母只是一个归一化项,确保权重加起来等于 1。上式给出的方案简单好用,但一个
...
#+NAME: ch02-code-afc867a976777c48
#+BEGIN_SRC python
...
[fn:1] 参见 [[https://www.countbayesie.com/blog/2015/2/18/bayes-theorem-with-lego]]。

[fn:2] 这里我们省略了与数据采集本身相关的任务,但实验设计的重要性丝毫不亚于统计分析的其他
  方面,详见第 9 章。

[fn:3] [[https://arviz-devs.github.io/arviz/]]

[fn:4] 该例子改编自 [[https://mc-stan.org/users/documentation/case-studies/golf.html]] 以及
  [[https://docs.pymc.io/notebooks/putting_workflow.html]]。

[fn:5] 该例子改编自参考文献 Gelman2020。

[fn:6] 逻辑回归模型的细节见第 3 章。

[fn:7] 后验预测检验是一个非常通用的思路,这些图并不是唯一的选择,只是 ArviZ 提供的部分选项。

[fn:8] 除非你意识到需要重新收集数据,但那是另一个故事了。

[fn:9] 可以试试 [[https://www.timeanddate.com/sun/ecuador/quito]]。

[fn:10] 不要和均值 MCSE 的标准差混淆。

[fn:11] 目前最常用、最有效的贝叶斯推断采样方法大多是 HMC 的变体,包括 PyMC 对连续变量默认
  使用的方法。

[fn:12] 一个除零点外处处为零、在零点取值无穷大的函数。

[fn:13] 对像序贯蒙特卡洛(SMC)这样的采样器而言,增加抽样数量同时也会增加粒子数,因此确实
  可能带来更好的收敛效果。

[fn:14] 严格来说离散模型应该使用"概率"而不是"密度",但这个区分在实践中很快会变得繁琐。

[fn:15] 在非贝叶斯语境下,$\boldsymbol{\theta}$ 是一个点估计,例如通过最大化似然得到。

[fn:16] 我们同时也会计算后验预测分布的样本,用来计算 LOO-PIT。

[fn:17] 在本书写作时,这个方法尚未在 ArviZ 中实现,但你阅读本书时它可能已经可用。

[fn:18] 可参考案例研究 [[https://avehtari.github.io/modelselection/roaches.html]]。

[fn:19] 关于概率积分变换更深入的介绍,请参阅相关章节。

[fn:20] 赤池信息量准则(AIC)是泛化误差的一种估计量,在频率学派统计中很常用,但其假设通常
  不适合直接套用在贝叶斯模型上。

[fn:21] 这个公式对 WAIC 等其他信息准则同样适用。
