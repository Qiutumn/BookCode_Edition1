...
:width: 8.00in
相对变量重要性。左图：前两个输入变量对预测变量有贡献，其余变量是噪声。中图：前 5 个变量与
输出变量有关。右图：10 个输入变量与预测变量完全无关。黑色虚线表示“所有变量同等重要”时的变量
重要性数值。
```

从 {numref}`fig:bart_vi_toy` 还能看到增加树数量 $m$ 的影响。一般来说，随着 $m$ 增大，相对
重要性的分布会变得更**平坦**。这是一个有直观解释的已知现象：$m$ 越大，对每棵树预测能力的要求
越低，因此相关性较弱的特征也更有机会进入某棵树。反之，降低 $m$ 会提高对每棵树的要求，从而在
变量之间引入更严格的“竞争”；结果是，只有**真正重要**的变量才会进入最终的树。

像 {numref}`fig:bart_vi_toy` 这样的图可以帮助区分较重要和较不重要的变量
{cite:p}`ChipmanBARTBayesianadditive2010, Carlson2020`。一种做法是观察从较小 $m$ 增加到较大
$m$ 时会发生什么：如果相对重要性下降，该变量就**更重要**；如果变量重要性上升，它就**较不
重要**。例如，在第一幅图中，无论 $m$ 取什么值，前两个变量显然都比其余变量重要得多；第二幅图
也可以对前 5 个变量得出类似结论。最后一幅图中，所有变量都同等（不）重要。
...
:width: 8.00in
使用不同树数量拟合的 BART 所给出的相对变量重要性。小时是最重要的协变量，其次是温度；湿度和
风速看起来相关性较弱。
```
""",
    ),
