...
def _inline_md_to_org(s):
    ...
    # bold **text** -> *text*
    s = re.sub(r"\*\*([^*]+)\*\*", r"*\1*", s)
    # italic _text_ -> /text/ (but NOT intraword underscores like
    # "BookCode_Edition1" or inside URLs -- require the underscore to sit
    # on a word boundary, same as CommonMark's rule for _-emphasis)
