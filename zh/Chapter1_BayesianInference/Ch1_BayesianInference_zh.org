...
本仓库 fork 自 [[https://github.com/BayesianModelingandComputationInPython/BookCode_Edition1][BayesianModelingandComputationInPython/BookCode_Edition1]]。
根据原仓库 =welcome.md= 的许可说明,书籍正文采用
[[https://creativecommons.org/licenses/by-nc-sa/4.0/][CC BY-NC-SA 4.0]],代码(包括代码块与
Jupyter Notebook)采用 [[https://www.gnu.org/licenses/old-licenses/gpl-2.0.html][GPL-2.0]]。
...
- 未知量用概率分布来描述 [fn:1]。我们把这些量称为*参数(parameters)*[fn:2]。
...
*之前*,我们对参数存在的不确定性 [fn:3]。把二者相乘,就得到了后验分布,也就是模型中
所有参数(在给定观测数据条件下)的联合分布。下面这段代码复现了书中图 1.1 的思路:给定
一个任意的先验和似然,画出对应的后验 [fn:4]。

注意,虽然 $\boldsymbol{Y}$ 是观测数据,但它本身也是一个随机向量,因为它的取值依赖于
某次具体实验的结果 [fn:5]。为了得到后验分布,我们把数据当作固定在实际观测值上。正因如此,
...
如果 $f$ 是恒等函数,那么 $J$ 就是 $\boldsymbol{\theta}$ 的均值 [fn:7]:
...
#+NAME: ch01-code-e4f9d0fd7ac9fb63
#+BEGIN_SRC python
%matplotlib inline
import os
...
#+RESULTS: stderr
#+BEGIN_EXAMPLE
/home/ai/Projects/BookCode_Edition1/.venv/lib/python3.12/site-packages/arviz/__init__.py:50: FutureWarning: 
...
#+NAME: ch01-code-2093d1b92b303e6a
#+BEGIN_SRC python
...
#+NAME: ch01-code-3db733dea9698000
#+BEGIN_SRC python
...
#+RESULTS: stderr
#+BEGIN_EXAMPLE
/tmp/ipykernel_2611045/3933215254.py:5: RuntimeWarning: divide by zero encountered in log
  log_prior = np.log(prior)
#+END_EXAMPLE
[[file:generated/chapter-1-8e961ce60d-ch01-code-3db733dea9698000-39f966b219-figure.png]]
...
#+NAME: ch01-code-072118534c2731b0
#+BEGIN_SRC python
...
#+NAME: ch01-code-9d52d77492547808
#+BEGIN_SRC python
...
#+NAME: ch01-code-c303efe59ae354a4
#+BEGIN_SRC python
n_iters = 1000
...
#+NAME: ch01-code-30b02e098f238eef
#+BEGIN_SRC python
...
Density Interval, HDI)。HDI 是包含给定比例(此处是 94%)概率密度的最短区间 [fn:12]。下面
...
#+NAME: ch01-code-2aedea609a1ff58e
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-2aedea609a1ff58e-ff7be64a08-figure.png]]
...
#+NAME: ch01-code-ad0697df16764561
#+BEGIN_SRC python
az.plot_posterior(trace)
plt.savefig("img/chp01/plot_posterior.png")
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-ad0697df16764561-21fcb4415b-figure.png]]

#+NAME: ch01-code-e950d0569a6518a2
#+BEGIN_SRC python
# 用 PyMC 声明模型
...
#+NAME: ch01-code-e7bdeb7314fdba27
#+BEGIN_SRC python
import shutil
...
#+RESULTS: stdout
#+BEGIN_EXAMPLE
未找到 Graphviz 的 dot 可执行文件;以下显示等价的 DOT 源码:
...
#+NAME: ch01-code-bf15676d3007df91
#+BEGIN_SRC python
...
#+RESULTS: stdout
#+BEGIN_EXAMPLE
跳过 PNG 渲染:系统未安装 Graphviz 的 dot 可执行文件。
#+END_EXAMPLE
...
#+NAME: ch01-code-1ae9067b8340806d
#+BEGIN_SRC python
pred_dists = (
...
#+NAME: ch01-code-ed7a613a62f93fec
#+BEGIN_SRC python
...
#+END_EXAMPLE
[[file:generated/chapter-1-8e961ce60d-ch01-code-ed7a613a62f93fec-029b4780fb-figure.png]]
...
#+NAME: ch01-code-21d973a1cc8e69f9
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-21d973a1cc8e69f9-db33c402f8-figure.png]]
...
解,不需要复杂的计算 [fn:14]。但从现代计算的角度看,共轭先验通常并不比其他选择更好——主要
...
#+NAME: ch01-code-72b477ec3ce618c2
#+BEGIN_SRC python
...
#+RESULTS: stderr
#+BEGIN_EXAMPLE
/tmp/ipykernel_2611045/2688038698.py:19: RuntimeWarning: invalid value encountered in divide
  axes[idx].plot(np.divide(y, N), 0, color='k', marker='o', ms=12)
#+END_EXAMPLE
[[file:generated/chapter-1-8e961ce60d-ch01-code-72b477ec3ce618c2-48568ff762-figure.png]]
...
方法的结果,都可以理解为在某些特定先验下的贝叶斯 MAP 估计 [fn:15]。
...
来推算出 $\kappa$ [fn:16],要么自己为 $\kappa$ 选一个先验来算后验。JP 保证的是:如果 Alice
...
#+NAME: ch01-code-f0b994a5cc232053
#+BEGIN_SRC python
...
#+RESULTS: stderr
#+BEGIN_EXAMPLE
/tmp/ipykernel_2611045/588671948.py:2: RuntimeWarning: divide by zero encountered in divide
  κ = (θ / (1-θ))
/tmp/ipykernel_2611045/588671948.py:10: RuntimeWarning: divide by zero encountered in power
  axes[0, 0].plot(θ, θ**(-0.5) * (1-θ)**(-0.5))
/tmp/ipykernel_2611045/588671948.py:15: RuntimeWarning: divide by zero encountered in power
  axes[0, 1].plot(κ, κ**(-0.5) * (1 + κ)**(-1))
/tmp/ipykernel_2611045/588671948.py:17: RuntimeWarning: invalid value encountered in multiply
  axes[1, 1].plot(κ, κ**(y-0.5) * (1 + κ)**(-n-1))
#+END_EXAMPLE
[[file:generated/chapter-1-8e961ce60d-ch01-code-f0b994a5cc232053-6a06231583-figure.png]]


注意,由于上式中的期望是对 $Y \mid \theta$ 求的,也就是对*样本空间*求期望。这意味着,
要得到一个 JP,我们需要对所有可能的实验结果取平均。这违反了似然原理(likelihood
principle)[fn:17]——因为这样一来,关于 $\theta$ 的推断不仅依赖于手头的数据,还依赖于一整
...
区分,那么这样的先验就是在合理取值范围上的均匀分布 [fn:18]。但如果我们并非对所有取值都
...
#+NAME: ch01-code-bba82424fab9a8db
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-bba82424fab9a8db-44eabd6169-figure.png]]
...
#+NAME: ch01-code-1c0a69ab965f38db
#+BEGIN_SRC python
ite = 100_000
...
#+NAME: ch01-code-5aab4b4a3cefc72f
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-5aab4b4a3cefc72f-637ed8b41a-figure.png]]


在以下约束条件下,熵最大的分布分别是 [fn:19]:
...
宽泛、模糊的态度;最后一个是信息先验,紧紧围绕 $\theta=0.8$ [fn:20]。如果我们确实拥有来自
...
#+NAME: ch01-code-a9b85cddc6070bf6
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-1-8e961ce60d-ch01-code-a9b85cddc6070bf6-f3adb3a8d2-figure.png]]
...
[fn:1] 如果想说得更宽泛一些,甚至可以说万事万物都是一个概率分布——只不过对于那些我们
  假定能以任意精度确切知道的量,这个分布是一个狄拉克 δ 函数。

[fn:2] 有些学者把这类量称为潜变量(latent variables),而把"参数"这个名字留给那些固定
  但未知的量。

[fn:3] 你也可以换个角度,从确定性或信息量的角度来理解这一点——取决于你是"半杯水是空的"
  还是"半杯水是满的"那种性格。

[fn:4] 有时"分布"这个词会被省略,在讨论这些话题时这种省略很常见。

[fn:5] 这里的"实验"是广义的,指任何采集或生成数据的流程。

[fn:6] 参见 [[https://xkcd.com/2117/]] 这幅关于积分的漫画。

[fn:7] 严格来说应该讨论的是随机变量的期望。

[fn:8] 参见马尔可夫链与 Metropolis-Hastings 相关章节中关于细致平衡(detailed balance)
  的讨论。

[fn:9] 关于推断方法更详尽的讨论,建议阅读原书相关章节及其参考文献。

[fn:10] 在其他通用推断引擎中,这有时也被称为"核"(kernel)。

[fn:11] 你也可以用 ArviZ 的 =plot_trace= 函数得到类似的图,本书后续都会这样做。

[fn:12] 注意,原则上包含给定比例总密度的区间有无穷多个。

[fn:13] 更多例子可参见维基百科的共轭先验分布表。

[fn:14] 除了那些发生在你脑子里的计算之外。

[fn:15] 举例来说,带 L2 正则化的线性回归,等价于对回归系数使用高斯先验。

[fn:16] 举例来说,如果我们有 $\theta$ 的后验样本,就可以把这些样本代入
  $\kappa = \frac{\theta}{1-\theta}$ 直接算出对应的 $\kappa$ 样本。

[fn:17] 参见维基百科"似然原理"(Likelihood principle)词条。

[fn:18] 参见前文关于"熵"的讨论。

[fn:19] 维基百科"最大熵概率分布"词条中有更长的列表。

[fn:20] 即便这类先验的严格定义需要比这里所给的更多语境,我们依然认为这个例子能传达一种
