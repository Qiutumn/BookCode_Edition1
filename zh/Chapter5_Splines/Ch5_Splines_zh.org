...
本章讨论样条(spline)。它扩展了第 [[chap2][3]] 章介绍的概念,目的是为模型增加更大的灵活性。第
[[chap2][3]] 章中的模型假定因变量与自变量之间的关系在整个定义域上保持同一种形式。与此不同,
...
#+NAME: ch05-imports-profiles-paths
#+BEGIN_SRC python
import os
...
均值。式 [[eq:lin_model]] 可以改写为:

<<eq:polynomial_reg>>
#+NAME: eq:polynomial_reg
\[
\mathbb{E}[Y]= \beta_0 + \beta_1 X + \beta_2 X^2 + \cdots + \beta_m X^m
\]

这称为*多项式回归(polynomial regression)*。乍看之下,式 [[eq:polynomial_reg]] 似乎表示
以 $X,X^2,\cdots,X^m$ 为协变量的多元线性回归。这种理解在某种意义上没有错,但必须注意:
所有协变量 $X^m$ 都来自同一个 $X$,只是依次取了从 1 到 $m$ 次幂。因此就实际问题而言,我们仍然
只拟合了一个预测变量。

$m$ 称为多项式的次数(degree)。第 [[chap2][3]] 章和第 [[chap3][4]] 章的线性回归模型都是 1 次
多项式,唯一的例外是 [[transforming_covariates]] 节中方差随协变量变化的例子,那里使用了
$m=1/2$。

[[fig:polynomial_regression]] 给出了次数为 2、10 和 15 的 3 个多项式回归例子。随着次数
增加,曲线会变得更灵活。

#+NAME: ch05-polynomial-data
#+BEGIN_SRC python
...
#+NAME: ch05-figure-polynomial-regression
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-polynomial-regression-b372127200-ch05-figure-polynomial-regression-b372127200-figure.png]]
...
<<fig:polynomial_regression>>
#+NAME: fig:polynomial_regression
#+CAPTION: 次数为 2、10 和 15 的多项式回归示例。次数越高,拟合越/曲折(wiggly)/。蓝色实线使用全部 观测,橙色虚线表示删除橙色叉号所标观测后的拟合。删除一个数据点对 2 次和 10 次多项式的影响较小, 但对 15 次多项式的影响较大。 拟合使用最小二乘法计算。
[[file:figures/polynomial_regression.png]]

多项式的一个问题是它会产生/全局/作用。使用 $m$ 次多项式,等于声称自变量与因变量之间在整个
数据集上的关系都是 $m$ 次的。当数据的不同区域需要不同程度的灵活性时,这会造成问题,例如得到
/过于灵活/的曲线 [fn:1]。在 [[fig:polynomial_regression]] 的最后一个面板(次数为 15)中,
即使高 $X$ 区域没有取值如此低或如此高的数据点,拟合曲线仍出现了一个/深谷/,紧接着又出现一个
/高峰/。

此外,随着次数增加,拟合对删除数据点会越来越敏感；等价地说,它也会对未来新增的数据越来越敏感。
换言之,次数越高,模型越容易过拟合。例如在 [[fig:polynomial_regression]] 中,实线表示
...
把特征扩展的思想推广到多项式之外,可以把式 [[eq:lin_model]] 展开为:
...
对数或平方根等函数。这些函数可以由具体问题所启发。例如在 [[transforming_covariates]] 节中,
...
*分段多项式(piecewise polynomial)* [fn:2],如 [[fig:piecewise]] 所示。

#+NAME: ch05-figure-piecewise-polynomials
#+BEGIN_SRC python
piecewise_figure, _ = plot_piecewise_splines([1.57, 4.71])
piecewise_figure.savefig(FIGURE_DIR / "piecewise.png", bbox_inches="tight")
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-piecewise-polynomials-bd91d22cb7-ch05-figure-piecewise-polynomials-bd91d22cb7-figure.png]]

<<fig:piecewise>>
#+NAME: fig:piecewise
#+CAPTION: 蓝线是我们希望逼近的/真实/函数。黑色实线是次数依次增加的分段多项式(0、1、2 和 3),分别 对应分段常数、分段线性、分段二次和分段三次拟合。灰色竖虚线标出 x 轴上各子定义域的边界。
[[file:figures/piecewise.png]]

[[fig:piecewise]] 的 4 个面板有同一个目标:逼近蓝色函数。首先用灰色虚线把函数拆成 3 个
...
$X_0,X_1,\cdots,X_p$,甚至可以再加入逆链接函数 $\phi$ [fn:5]。这种形式的模型称为**广义加性模型
(Generalized Additive Model, GAM)** [cite:@GelmanBayesianDataAnalysis2013; @WoodGeneralizedAdditiveModels2017]:
...
良好的模型。定义样条时需要给出*节点(knot)* [fn:7]。节点的作用是把变量 $\boldsymbol{X}$ 的
定义域拆成一系列相邻区间。例如,[[fig:piecewise]] 中灰色竖虚线所表示的就是节点。对本章
而言,样条是受到连续性约束的分段多项式,也就是强制两个相邻子多项式在节点处相接。如果子多项式
次数为 $n$,就称该样条为 $n$ 次样条。有时也按阶数(order)称呼样条,此时阶数为 $n+1$。

从 [[fig:piecewise]] 可以看到,随着分段多项式次数增加,结果函数的/平滑度/也会提高。前面
...
都可以由同次数 B 样条的线性组合构成。[[fig:splines_basis]] 给出了次数从 0 到 3(从上到下)
的 B 样条例子。底部圆点表示节点,蓝点标出了加粗黑色 B 样条不为零的区间。为清晰起见,其他
B 样条都画成较细的虚线,但所有 B 样条同等重要。事实上,[[fig:splines_basis]] 的每个子图
都展示了由给定节点所定义的全部 B 样条。换言之,一组节点和一个次数就能完全确定 B 样条。

#+NAME: ch05-figure-bspline-bases
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bspline-bases-a2de998a9d-ch05-figure-bspline-bases-a2de998a9d-figure.png]]
...
#+CAPTION: 次数从 0 到 3 的 B 样条。最上方子图是阶梯函数,第二幅是三角函数,再往下则越来越接近高斯形状。 边界处/堆叠/的节点(较小黑点)用于定义靠近边界的样条。
[[file:figures/splines_basis.png]]

从 [[fig:splines_basis]] 可以看到,随着 B 样条次数提高,单个 B 样条覆盖的定义域会越来越宽
[fn:8]。因此,要让高次样条有意义,需要定义更多节点。还要注意,每个 B 样条只在某个给定区间内
非零。正是这一性质,使样条回归比多项式回归更具有/局部性/。

随着次数增加,控制每个 B 样条所需的节点数也会增加。因此对所有大于 0 的次数,边界附近一开始
无法定义完整的 B 样条。这也解释了为什么在 [[fig:splines_basis]] 中,随着次数提高,黑色
...
[[fig:splines_basis]] 最后一个面板里形似指数分布的靛蓝虚线。随后用 $(0,0,0,1,2)$ 定义
...
#+NAME: ch05-figure-weighted-bsplines
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-weighted-bsplines-bde337be7c-ch05-figure-weighted-bsplines-bde337be7c-figure.png]]

<<fig:splines_weighted>>
#+NAME: fig:splines_weighted
#+CAPTION: 用 Patsy 定义的 B 样条。第一行从左到右分别是阶数为 1(分段常数)、2(分段线性)和 4(三次)的 样条；每个基函数以不同颜色的虚线表示。第二行把第一行的基样条按一组系数缩放。 粗黑线表示这些基函数之和。因为系数是随机选取的,第二行每个子图都可以看成从/样条空间/上的先验 分布抽取的一个随机样本。
[[file:figures/splines_weighted.png]]

本例中的 $\beta_i$ 系数来自半正态分布,对应代码块
[[splines_patsy_plot][splines_patsy_plot]] 中生成正系数的步骤。因此,
[[fig:splines_weighted]] 的每个面板只展示了样条概率分布的一次实现。删除随机种子并多次运行
该代码块,每次都会看到不同的样条。还可以把半正态分布替换为正态分布、指数分布等。
[[fig:splines_realizations]] 展示了三次样条的 4 次实现。

#+NAME: ch05-figure-cubic-realizations
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-cubic-realizations-303e2dea60-ch05-figure-cubic-realizations-303e2dea60-figure.png]]

<<fig:splines_realizations>>
#+NAME: fig:splines_realizations
#+CAPTION: $\beta_i$ 系数从半正态分布抽取时,三次样条的 4 次实现。
[[file:figures/splines_realizations.png]]

::: {admonition} 对样条而言,四阶已经显得拥挤

在所有可能的样条中,三次样条或许最常用。但三次样条为什么堪称样条中的“女王”?
[[fig:piecewise]] 和 [[fig:splines_weighted]] 提供了一些线索。对大多数常见场景而言,
...
斜率连续,而且“斜率的斜率”也连续 [fn:9]。事实上,$m$ 次样条在节点处会有 $m-1$ 阶连续导数。
...
在 [[fig:splines_basis]] 和 [[fig:splines_weighted]] 中已经画出了 B 样条,但还没有
说明如何计算它们。主要原因是手工计算相当繁琐,而 SciPy 等软件包已经提供了高效算法 [fn:10]。
...
="y ~ x1 + x2"=,若要加入交互项则可以写成 ="y ~ x1 + x2 + x1:x2"=。这与第 [[chap2][3]] 章
介绍 Bambi 的边注中所示语法相似。更多细节见 Patsy 文档 [fn:11]。
...
[[fig:design_matrices]] 展示了代码块 [[splines_patsy][splines_patsy]] 计算出的 3 个设计矩阵。
...
#+NAME: ch05-patsy-design-matrices-small
#+BEGIN_SRC python
...
#+NAME: ch05-figure-design-matrices
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-design-matrices-d688a7ffa8-ch05-figure-design-matrices-d688a7ffa8-figure.png]]

<<fig:design_matrices>>
#+NAME: fig:design_matrices
#+CAPTION: 代码块 [[splines_patsy][splines_patsy]] 中 Patsy 生成的设计矩阵。颜色从黑色(1)过渡到浅灰色(0)； 列数是 B 样条数量,行数是数据点数量。
[[file:figures/design_matrices.png]]

[[fig:design_matrices]] 的第一个子图对应 0 次样条 =B0=。可以看到,设计矩阵只包含 0(浅灰)
和 1(黑色)。第一个 B 样条(第 0 列)对前 5 个观测取 1,其余取 0；第二个 B 样条(第 1 列)对前
5 个观测取 0,对接下来 5 个取 1,之后又回到 0。同样的模式继续重复。把它与
[[fig:splines_weighted]] 第一行的第一个子图比较,就能看出设计矩阵如何编码那幅图。

[[fig:design_matrices]] 的第二个子图中,第一个 B 样条从 1 降到 0；第二、第三和第四个先从
0 升到 1,再从 1 降到 0；第五个从 0 升到 1。这些模式分别对应
[[fig:splines_weighted]] 第一行第二个子图中的负斜率直线、3 个三角函数和正斜率直线。

类似地,可以比较 [[fig:design_matrices]] 第三个子图的 7 列与
[[fig:splines_weighted]] 第一行第三个子图的 7 条曲线。

代码块 [[splines_patsy][splines_patsy]] 用于生成 [[fig:splines_weighted]] 和
[[fig:design_matrices]] 中的 B 样条。唯一差别是前者使用
...
单车数据集 [fn:12]。目标是估计 24 小时内每个小时租出的自行车数量。先加载并绘制数据。

#+NAME: ch05-load-bike-data
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bike-data-52975a9c6a-ch05-figure-bike-data-52975a9c6a-figure.png]]

<<fig:bikes_data>>
#+NAME: fig:bikes_data
#+CAPTION: 自行车数据的可视化。每个点表示一天某个小时(0 到 23)内标准化后的租车数量。点使用半透明样式, 以减少过度重叠,从而更容易看清数据分布。
[[file:figures/bikes_data.png]]

快速查看 [[fig:bikes_data]] 就能发现,一天中的小时与租车数量之间的关系很难用一条直线
...
这个样条回归模型与第 [[chap2][3]] 章的线性模型非常相似。所有困难工作都由设计矩阵
$\boldsymbol{B}$ 及其对特征空间的扩展完成。这里用线性代数记号更简洁地表示式 [[eq:bfr]] 和
[[eq:bfr2]] 中的乘法与求和,即写成
...
#+NAME: ch05-figure-bike-weighted-bases
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bike-weighted-bases-b4d376d336-ch05-figure-bike-weighted-bases-b4d376d336-figure.png]]
...
[[fig:bikes_spline_raw_data]] 用黑色实线展示最终线性预测,并用虚线展示每个加权 B 样条。
这种表示很直观,因为它同时揭示了各个 B 样条对最终结果的贡献。

<<fig:bikes_spline_raw_data>>
#+NAME: fig:bikes_spline_raw_data
#+CAPTION: 用样条拟合自行车数据。虚线表示加权 B 样条,它们的和构成较粗的黑色实线。所绘数值是后验均值, 黑点表示节点。与 [[fig:splines_weighted]] 相比,这里的样条看起来更/锯齿状/。原因是本图 只在较少位置计算函数:数据按小时分箱,因此这里只含 24 个不同小时,而 [[fig:splines_weighted]] 使用 500 个位置。
[[file:figures/bikes_spline_raw_data.png]]

当目标是展示模型结果时,把原始数据与样条及其不确定性叠加在一起通常更有用,如
[[fig:bikes_data2]] 所示。从图中很容易看出,深夜租车数量最低；随后数量上升,可能对应人们
起床通勤。大约 10 点出现第一个峰值,之后趋于平缓或略有下降；大约 18 点人们下班回家时出现
第二个峰值,再往后则持续下降。

#+NAME: ch05-figure-bike-posterior-hdi
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bike-posterior-hdi-9ad51ae278-ch05-figure-bike-posterior-hdi-9ad51ae278-figure.png]]

<<fig:bikes_data2>>
#+NAME: fig:bikes_data2
#+CAPTION: 用样条拟合自行车数据(黑点)。阴影表示均值函数的 94% 最高密度区间(highest density interval, HDI),蓝线表示平均趋势。
[[file:figures/bikes_spline_data.png]]
...
[[tab:loo_splines]] 明确标为*原书历史发布结果*:当时类似代码块 [[splines][splines]] 的模型
分别使用 3、6、9、12 和 18 个等距节点,LOO 把 12 节点样条排在第一。表中 =m_12k= 的堆叠权重为
0.88,=m_3k= 为 0.12,其余模型接近 0。正如 [[model_averaging]] 节所解释的,堆叠试图组合多个
...
[[fig:bikes_spline_loo_knots]] 展示当前运行中这些模型的后验平均样条,并根据当前比较结果决定强调
哪条曲线,不把历史排名硬编码进图。

#+NAME: ch05-bike-knot-comparison-models
#+BEGIN_SRC python
...
<<tab:loo_splines>>
#+CAPTION: 原书历史发布运行中使用 LOO 比较不同节点数量样条模型的摘要。
|  | *rank* | *loo* | *p_loo* | *d_loo* | *weight* | *se* | *dse* | *warning* | *loo_scale* |
| m_12k | 0 | -377.67 | 14.21 | 0.00 | 0.88 | 17.86 | 0.00 | False | log |
| m_18k | 1 | -379.78 | 17.56 | 2.10 | 0.00 | 17.89 | 1.45 | False | log |
| m_9k | 2 | -380.42 | 11.43 | 2.75 | 0.00 | 18.12 | 2.97 | False | log |
| m_6k | 3 | -389.43 | 9.41 | 11.76 | 0.00 | 18.16 | 5.72 | False | log |
| m_3k | 4 | -400.25 | 7.17 | 22.58 | 0.12 | 18.01 | 7.78 | False | log |

#+NAME: ch05-figure-bike-loo-knots
#+BEGIN_SRC python
figure, axis = plt.subplots(figsize=(10, 4))
axis.scatter(
    data["hour"],
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bike-loo-knots-6c69f44777-ch05-figure-bike-loo-knots-6c69f44777-figure.png]]

<<fig:bikes_spline_loo_knots>>
#+NAME: fig:bikes_spline_loo_knots
#+CAPTION: 代码块 [[splines][splines]] 所描述模型在使用不同节点数(3、6、9、12、18)时的后验平均样条。 图对应当前 =CH5_PROFILE= 运行:当前 LOO 排名第一的模型以蓝色粗实线强调,其余模型以不同线型的 灰线显示。图例直接标出当前第一名,因此不会把 [[tab:loo_splines]] 的历史排名硬编码进新图。 =smoke= 图仅用于执行检查,模型选择应查看新的 =release= 输出。
[[file:figures/bikes_spline_loo_knots.png]]
...
正则化先验。从样条定义和 [[fig:splines_weighted]] 可以看到,相邻
$\boldsymbol{\beta}$ 系数越接近,结果函数越平滑。想象从
[[fig:splines_weighted]] 的设计矩阵中删除相邻两列,等价于把相应系数设为 0。因为预测变量
...
#+NAME: eq:spline_regularized_prior
\[
\begin{aligned}
\begin{split}
...
#+NAME: eq:spline_regularized_GW_prior
\[
\begin{aligned}
\begin{split}
\tau\sim& \mathcal{N}(0, 1) \\
...
#+NAME: ch05-bike-regularized-design
#+BEGIN_SRC python
...
#+NAME: ch05-bike-unregularized-model
#+BEGIN_SRC python
...
#+NAME: ch05-figure-bike-grw-comparison
#+BEGIN_SRC python
wiggly_mean = (
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-bike-grw-comparison-70a9db790c-ch05-figure-bike-grw-comparison-70a9db790c-figure.png]]

从 [[fig:bikes_spline_data_grw]] 可以看到,=bike_grw_model= 的蓝色实线比
=bike_wiggly_model= 的灰色虚线更少曲折,虽然这里的差异确实比较小。

<<fig:bikes_spline_data_grw>>
#+NAME: fig:bikes_spline_data_grw
#+CAPTION: 使用独立高斯先验或正则化高斯随机游走先验(GRW)拟合自行车数据。两个模型都使用 22 个内部节点。 灰色虚线是 =bike_wiggly_model= 的独立高斯系数先验拟合,蓝色实线是 =bike_grw_model= 的 GRW 平滑先验拟合；图例同时给出线型编码。
[[file:figures/bikes_spline_data_grw.png]]
...
作为最后一个样条例子,我们使用一项实验研究的数据 [cite:@Potvin1990; @Pedersen2019]。实验在
...
#+NAME: ch05-co2-global-design-matrix
#+BEGIN_SRC python
num_knots = 2
...
模型设计漫长过程的一条捷径 [cite:@Gelman2020]。除了无需从零构思模型这一明显优势之外,它还
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-co2-global-cc30cc4970-ch05-figure-co2-global-cc30cc4970-figure.png]]

从 [[fig:sp_global]] 可以清楚看到,模型只对部分植物提供了良好拟合。把所有植物合并在一起时,
模型平均而言表现不错,但对具体植物的拟合并不好。

<<fig:sp_global>>
#+NAME: fig:sp_global
#+CAPTION: 黑点表示 12 株植物(Qn1、Qn2、Qn3、Qc1、Qc2、Qc3、Mn1、Mn2、Mn3、Mc1、Mc2、Mc3)分别在 7 个 CO₂ 浓度下测得的 CO₂ 吸收量。黑线是代码块 [[sp_global][sp_global]] 中模型的平均样条拟合, 灰色阴影表示该拟合的 94% HDI。
[[file:figures/sp_global.png]]
...
#+NAME: ch05-co2-individual-design-matrix
#+BEGIN_SRC python
Bi = np.asarray(
    dmatrix(
        "bs(conc, knots=knots, degree=3, include_intercept=True) - 1",
...
#+NAME: ch05-co2-individual-model
#+BEGIN_SRC python
...
#+NAME: ch05-figure-co2-individual
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-co2-individual-ced65798a6-ch05-figure-co2-individual-ced65798a6-figure.png]]

从 [[fig:sp_individual]] 可以看到,现在对每株植物的拟合都好得多。

<<fig:sp_individual>>
#+NAME: fig:sp_individual
#+CAPTION: 12 株植物在 7 个 CO₂ 浓度下测得的 CO₂ 吸收量。黑线是代码块 [[sp_individual][sp_individual]] 中模型的平均样条拟合,灰色阴影表示该拟合的 94% HDI。
[[file:figures/sp_individual.png]]

也可以混合前两个模型 [fn:13]。如果希望同时估计 12 株植物的全局趋势和各自拟合,这种做法会很有
...
#+NAME: ch05-figure-co2-mixed-decomposition
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-co2-mixed-decomposition-09a08f5c74-ch05-figure-co2-mixed-decomposition-09a08f5c74-figure.png]]

[[fig:sp_mix_decomposed]] 展示 =sp_mix= 模型的拟合。这个模型的一个优势是,可以把单株植物
拟合(蓝色)分解成两部分:黑色的全局趋势,以及灰色的单株偏离。注意,黑色全局趋势会在每个子图
重复。偏离不仅体现在平均吸收量上,也就是说它们不是水平直线；不同植物的函数响应形状也存在程度
不一的差异。

<<fig:sp_mix_decomposed>>
#+NAME: fig:sp_mix_decomposed
#+CAPTION: 12 株植物在 7 个 CO₂ 浓度下测得的 CO₂ 吸收量。蓝线是代码块 [[sp_mix][sp_mix]] 中模型的平均 样条拟合,灰色阴影表示 94% HDI。拟合被分解为两项:黑线与深灰带表示全局贡献,灰线与浅灰带表示 相对全局贡献的偏离。蓝线和蓝色带是全局趋势与偏离之和。
[[file:figures/sp_mix_decomposed.png]]
...
当前代码会为 3 个模型分别重新计算逐点 PSIS-LOO,再生成 [[fig:sp_compare]]。排名、标准误、
...
在真实分析中应检查当前输出中的警告,并尝试 [[k-paretto]] 节所述措施。

#+NAME: ch05-co2-loo-comparison
#+BEGIN_SRC python
co2_models = {
...
#+RESULTS: output
#+BEGIN_EXAMPLE
              profile  rank    elpd_loo      p_loo   elpd_diff    weight  \
sp_global       smoke     0 -321.608336  24.998357    0.000000  0.966075   
sp_mix          smoke     1 -406.493298  94.204909   84.884962  0.000000   
sp_individual   smoke     2 -462.541679   0.828650  140.933343  0.033925   

                      se        dse  warning  max_pareto_k  pareto_k_cutoff  
sp_global       9.810505   0.000000     True           inf         0.231378  
sp_mix         10.538188  12.251381     True           inf         0.231378  
sp_individual  11.645950  15.040322     True           inf         0.231378  
#+END_EXAMPLE

#+NAME: ch05-figure-co2-loo-comparison
#+BEGIN_SRC python
...
#+END_SRC
[[file:generated/chapter-5-c7fabe6e7f-ch05-figure-co2-loo-comparison-ed4aee18dc-ch05-figure-co2-loo-comparison-ed4aee18dc-figure.png]]
...
误差线。比较所需逐观测对数似然已在所有模型的 ~pm.sample(..., > idata_kwargs={"log_likelihood": True})~ 中显式请求。
#+END_QUOTE

<<fig:sp_compare>>
#+NAME: fig:sp_compare
#+CAPTION: 当前 =CH5_PROFILE= 运行对 3 个 CO₂ 吸收模型(=sp_global=、=sp_individual=、中心化的 =sp_mix=)所作的 LOO 比较。模型按本次运行的预测准确度从高到低排列。空心圆表示 LOO 值,黑点表示 样本内预测准确度。黑色线段表示 LOO 计算的标准误；以三角形为中心的灰色线段表示各模型 LOO 与 本次最佳模型之差的标准误。=smoke= 图只验证绘图路径,实质结论须使用新的 =release= 输出。
[[file:figures/sp_compare.png]]
...
*5E2.* 重做 [[fig:polynomial_regression]],但这次拟合 0 次和 1 次多项式。它们是否看起来
与其他某类模型相似?提示:可以使用 GitHub 仓库中的代码。

*5E3.* 重做 [[fig:piecewise]],但改变一个或两个节点的位置。节点位置如何影响拟合?代码
可在 GitHub 仓库中找到。
...
2. 你能解释参数 ~include_intercept=True/False~ 与公式中的 =+1/-1= 分别做什么吗?尝试生成类似
   [[fig:splines_basis]] 和 [[fig:design_matrices]] 的图来帮助回答。
...
*5M11.* 使用样条拟合第 [[chap3][4]] 章中的婴儿回归例子。
...
[fn:1] 详情参见龙格现象(Runge's phenomenon)。泰勒定理也能说明这一点:多项式适合在某个给定点
  附近逼近函数,但未必适合整个定义域。如果仍觉得难以理解,可以观看
  [[https://www.youtube.com/watch?v=3d6DsjIBzJ4]]。

[fn:2] 分段函数(piecewise function)由多个子函数定义,每个子函数适用于定义域中的不同区间。

[fn:3] 第 [[chap6][7]] 章将讨论阶梯函数如何在贝叶斯加性回归树(Bayesian Additive Regression
  Tree, BART)中发挥核心作用。

[fn:4] 也可以从数值角度说明这个约束的合理性:它减少了求解时需要确定的系数数量。

[fn:5] 与往常一样,恒等函数是一个有效选择。

[fn:6] 其他基函数还包括小波或傅里叶级数,第 [[chap4][6]] 章会看到这些例子。

[fn:7] 节点也称为断点(break point),后者可能更容易记,但文献中仍普遍使用 knot 一词。

[fn:8] 当次数趋于无穷时,B 样条会覆盖整条实数轴；不仅如此,它还会收敛到高斯函数,见
...
[fn:11] [[https://patsy.readthedocs.io]]

[fn:12] [[https://archive.ics.uci.edu/ml/datasets/bike+sharing+dataset]]

[fn:13] 是的,这也称为混合效应模型(mixed-effects model)。你可能还记得第 [[chap3][4]] 章讨论过的
  相关概念。
